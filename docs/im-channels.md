...
SeeAgent 支持多个即时通讯平台，每个平台通过独立的适配器 (Adapter) 接入统一的消息网关 (MessageGateway)。
...
| 飞书 | ✅ 稳定 | WebSocket 长连接 | ❌ 不需要 | `pip install seeagent[feishu]` |
| 钉钉 | ✅ 稳定 | Stream 模式 (WebSocket) | ❌ 不需要 | `pip install seeagent[dingtalk]` |
| 企业微信 | ✅ 稳定 | HTTP 回调（智能机器人） | ⚠️ 需要公网 IP | `pip install seeagent[wework]` |
| QQ 官方机器人 | ✅ 稳定 | QQ 开放平台 API (WebSocket) | ❌ 不需要 | `pip install seeagent[qqbot]` |
| OneBot | ✅ 稳定 | OneBot v11 (WebSocket) | ❌ 不需要 | `pip install seeagent[onebot]` |
...
1. 启动 SeeAgent 后，在 Telegram 中找到你的机器人
...
1. 启动 SeeAgent，日志应显示 `Feishu adapter: WebSocket started in background`
2. 在飞书中搜索并打开机器人对话
3. 发送消息，观察日志和机器人回复
...
1. 启动 SeeAgent，日志应显示 `DingTalk Stream client starting...`
...
> ⚠️ **重要**：创建机器人时企业微信会立即向回调 URL 发送 GET 验证请求，所以必须先在本地配好并启动 SeeAgent，确保回调端口可从公网访问，然后再创建机器人。

1. **获取企业 ID (Corp ID)**: 进入 [企业微信管理后台](https://work.weixin.qq.com/) →「我的企业」→「企业信息」页面底部
2. **在 `.env` 中填好配置**（见下方 SeeAgent 配置），启动 SeeAgent
...
2. SeeAgent 被动回复初始 stream（`finish=false`，创建流式会话）
...
1. 在 `.env` 中配好参数，启动 SeeAgent，日志应显示 `WeWorkBot adapter started`
...
1. 启动 SeeAgent，日志应显示 `QQ Official Bot ready (user: 你的机器人名称)`
...
SeeAgent 通过 OneBot v11 协议与中间层通信。你需要先部署一个 OneBot 实现：
...
- **WebSocket 正向连接**: SeeAgent 连接到本地 OneBot 服务器，无需公网 IP
- 支持自动断线重连（指数退避策略，初始 1 秒，最大 60 秒）

### 验证方法

1. 先启动 OneBot 服务器（如 NapCat），确认 WebSocket 监听正常
2. 启动 SeeAgent，日志应显示 `OneBot adapter connected to ws://127.0.0.1:8080`
...
- **ChannelAdapter**: 基类定义在 `src/seeagent/channels/base.py`，各平台实现在 `src/seeagent/channels/adapters/`
- **MessageGateway**: 统一消息路由、会话管理、媒体预处理，定义在 `src/seeagent/channels/gateway.py`
- **deliver_artifacts**: Agent 工具，用于主动发送文件/图片/语音，定义在 `src/seeagent/tools/handlers/im_channel.py`
