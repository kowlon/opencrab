...
本文档全面梳理 SeeAgent 项目的提示词系统架构，包括静态提示词文件、动态生成逻辑、以及各组件的协作关系。
...
SeeAgent 是一个全能自进化AI助手，核心目标是成为一个真正对用户有帮助的助手。
...
- **Prompt Builder**：`src/seeagent/prompt/builder.py::build_system_prompt()`
- **预算裁剪**：`src/seeagent/prompt/budget.py`
- **记忆检索**：`src/seeagent/prompt/retriever.py::retrieve_memory()`
...
**位置**: `src/seeagent/core/agent.py` 中的 `PROMPT_COMPILER_SYSTEM`
...
**位置**: `src/seeagent/memory/extractor.py`
...
**位置**: `src/seeagent/core/brain.py` - `plan()`
...
**位置**: `src/seeagent/core/brain.py` - `generate_code()`
...
**位置**: `src/seeagent/core/brain.py` - `analyze_error()`
...
**位置**: `src/seeagent/core/agent.py` - `BASE_TOOLS`
...
| `src/seeagent/core/identity.py` | 代码 | 身份管理，提示词生成 |
| `src/seeagent/core/agent.py` | 代码 | 主类，动态提示词组装 |
| `src/seeagent/core/brain.py` | 代码 | LLM 交互，内置提示词 |
| `src/seeagent/memory/extractor.py` | 代码 | 记忆提取提示词 |
