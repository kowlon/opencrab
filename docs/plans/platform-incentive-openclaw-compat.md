# SeeAgent Platform — 激励机制 + OpenClaw 兼容方案
...
ClawHub 技能与 SeeAgent 都使用 **SKILL.md + YAML frontmatter**，核心格式高度兼容：
...
| 环境要求 | `metadata.seeagent.os` | `metadata.openclaw.requires` |
| 版本号 | `version` 字段 | 无（通过 git tag 管理） |
| 发布 | 平台 API | `clawhub publish` CLI |
| 安装 | 平台 API / git clone | `clawhub install` CLI |

**结论：格式本身高度兼容。ClawHub 技能的 SKILL.md 可直接被 SeeAgent 解析加载（忽略 metadata.openclaw 即可），反之亦然。唯一需要适配的是 `handler`/`tool-name` 映射（纯指令型技能不需要）。**
...
- Champion 无限邀请码
- 支持邀请链接：`https://seeagent.ai/invite/{code}`
- 支持活动批量邀请码（用于社区推广）
...
链接分享 `seeagent.ai/invite/{code}` 自动展示 Open Graph 预览图。
...
- `handler` + `tool-name` 仅 SeeAgent 原生工具需要
- 纯指令型 Skill 天然双兼容
...
### Phase 1 — metadata.seeagent 规范 + 本地解析 (3 天)
