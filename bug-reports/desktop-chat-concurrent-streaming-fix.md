...
`src/seeagent/core/agent.py` 中，以下字段在每次 `chat_with_session_stream()` 入口处被直接覆盖：
...
`src/seeagent/core/reasoning_engine.py` 中：
...
##### 1. `src/seeagent/main.py` — 确保 `_orchestrator` 始终初始化
...
##### 2. `src/seeagent/api/server.py` — 注入 pool
...
##### 3. `src/seeagent/api/routes/chat.py` — 核心改动
...
##### 4. `src/seeagent/agents/factory.py` — AgentInstancePool 新增方法
...
##### 5. `src/seeagent/api/routes/chat.py` — `_stream_chat` 中的 profile 解析
...
def _resolve_profile(agent_profile_id: str | None) -> AgentProfile:
    ...
    from seeagent.agents.profile import AgentProfile, ProfileStore
...
| 文件 | 改动类型 | 说明 |
|------|----------|------|
| `src/seeagent/main.py` | 修改 | 始终初始化 pool 或 orchestrator，传给 API server |
| `src/seeagent/api/server.py` | 修改 | `create_app` 接收并存储 `agent_pool` |
| `src/seeagent/api/routes/chat.py` | 修改 | `/api/chat`、`/api/chat/cancel`、`/api/chat/skip`、`/api/chat/insert` 从 pool 获取 Agent |
| `src/seeagent/agents/factory.py` | 修改 | `AgentInstancePool` 新增 `get_existing()` 方法 |
