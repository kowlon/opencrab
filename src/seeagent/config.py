...
class Settings(BaseSettings):
    ...
    # Agent 配置
    agent_name: str = Field(default="SeeAgent", description="Agent 名称")
    ...
    log_file_prefix: str = Field(default="seeagent", description="日志文件前缀")
    ...
    bug_report_endpoint: str = Field(
        default="https://feedback-seeagent.fzstack.com",
    ...
    # === SeeAgent Platform (Agent Hub / Skill Store) ===
    hub_api_url: str = Field(
        default="https://seeagent.ai/api",
        description="SeeAgent Platform API base URL for Agent Hub and Skill Store",
    )
    hub_api_key: str = Field(
        default="",
        description="SeeAgent Platform API Key (ak_live_...)",
    )
    ...
    @property
    def seeagent_home(self) -> Path:
        """用户数据根目录，优先使用 SEEAGENT_ROOT 环境变量，默认 ~/.seeagent"""
        import os
    ...
    def user_workspace_path(self) -> Path:
        ...
        如果 project_root 位于 seeagent_home/workspaces/ 下（生产模式），
        直接使用 project_root 作为工作区路径；否则（开发模式）回退到 default。
        """
        ws_dir = self.seeagent_home / "workspaces"
        try:
    ...
    def skills_path(self) -> Path:
        """用户技能安装目录 (~/.seeagent/workspaces/default/skills)
    ...
    def mcp_builtin_path(self) -> Path:
        ...
        若不存在则回退到 wheel 打包位置 site-packages/seeagent/builtin_mcps/。
        """
