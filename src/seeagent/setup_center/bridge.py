...
- `python -m seeagent.setup_center.bridge list-providers`
- `python -m seeagent.setup_center.bridge list-models --api-type ... --base-url ... [--provider-slug ...]`
- `python -m seeagent.setup_center.bridge list-skills --workspace-dir ...`
...
import seeagent._ensure_utf8  # noqa: F401  # isort: skip
...
def list_providers() -> None:
    from seeagent.llm.registries import list_providers as _list_providers
...
async def _list_models_openai(api_key: str, base_url: str, provider_slug: str | None) -> list[dict]:
    ...
    from seeagent.llm.capabilities import infer_capabilities
...
def ensure_channel_deps(workspace_dir: str) -> None:
    ...
    from seeagent.python_compat import patch_simplejson_jsondecodeerror
    from seeagent.runtime_env import get_channel_deps_dir, get_python_executable, inject_module_paths_runtime
...
def list_skills(workspace_dir: str) -> None:
    ...
    for s in skills:
        ...
        if skill_path:
            try:
                origin_file = Path(skill_path) / ".seeagent-source"
...
def _try_platform_skill_download(skill_id: str, dest_dir: Path) -> bool:
    """Try downloading a cached skill ZIP from the SeeAgent platform.
    ...
    headers = {"User-Agent": "SeeAgent-SetupCenter"}
...
def _download_github_zip(repo_owner: str, repo_name: str, dest_dir: Path) -> None:
    ...
    for branch in ("main", "master"):
        ...
        for tpl in _GITHUB_ZIP_MIRRORS:
            ...
            try:
                req = urllib.request.Request(url, headers={"User-Agent": "SeeAgent"})
                with urllib.request.urlopen(req, timeout=30) as resp:
                    data = resp.read()
                break
            except Exception as e:
    ...
    with zipfile.ZipFile(io.BytesIO(data)) as zf:
        tmp_extract = Path(tempfile.mkdtemp(prefix="seeagent_zip_"))
        try:
...
def _download_gitee_zip(repo_owner: str, repo_name: str, dest_dir: Path) -> None:
    ...
    with zipfile.ZipFile(io.BytesIO(data)) as zf:
        tmp_extract = Path(tempfile.mkdtemp(prefix="seeagent_gitee_"))
        try:
            zf.extractall(tmp_extract)
            children = list(tmp_extract.iterdir())
            src = children[0] if len(children) == 1 and children[0].is_dir() else tmp_extract
...
def _read_skill_source(d: Path) -> str:
    """读取技能目录的安装来源标记。"""
    try:
        return (d / ".seeagent-source").read_text(encoding="utf-8").strip()
    except Exception:
        return ""
...
def install_skill(workspace_dir: str, url: str) -> None:
    ...
    if url.startswith("github:"):
        ...
        if _try_platform_skill_download(platform_skill_id, target):
            try:
                origin_file = target / ".seeagent-source"
        ...
        tmp_parent = Path(tempfile.mkdtemp(prefix="seeagent_skill_"))
    ...
    marketplace = [
        {
            ...
            "author": "seeagent",
            "url": "github:seeagent/skills/web-search",
            "stars": 42,
            "tags": ["搜索", "网络"],
        },
        {
            ...
            "author": "seeagent",
            "url": "github:seeagent/skills/code-interpreter",
            "stars": 38,
            "tags": ["代码", "数据分析"],
        },
        {
            ...
            "author": "seeagent",
            "url": "github:seeagent/skills/browser-use",
            "stars": 25,
            "tags": ["浏览器", "自动化"],
        },
        {
            ...
            "author": "seeagent",
            "url": "github:seeagent/skills/image-gen",
            "stars": 19,
            "tags": ["图片", "生成"],
        },
    ]
...
def main(argv: list[str] | None = None) -> None:
    ...
    p = argparse.ArgumentParser(prog="seeagent.setup_center.bridge")
...
if __name__ == "__main__":
    from seeagent.runtime_env import IS_FROZEN, ensure_ssl_certs, inject_module_paths
