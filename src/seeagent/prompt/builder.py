...
def build_system_prompt(
    ...
    bp_section = _build_bp_section(bp_session_id)
    if bp_section:
...
def _merge_policies(system: str, user: str) -> str:
    ...
    # 去掉用户文件可能的顶级标题 (# SeeAgent Policies 等)
    user_clean = re.sub(r"^#\s+[^\n]+\n*", "", user_clean).strip()

    if not system_titles:
        return f"# SeeAgent Policies\n\n{system}\n\n{user_clean}"
    ...
    parts = ["# SeeAgent Policies", "", system.strip()]
    if kept_sections:
        parts.append("")
        parts.append("\n\n".join(kept_sections))
    return "\n".join(parts)
...
- **SeeAgent 版本**: {version_str}
- **部署模式**: {deploy_mode}
- **当前时间**: {current_time}
- **操作系统**: {platform.system()} {platform.release()} ({platform.machine()})
- **当前工作目录**: {os.getcwd()}
- **SeeAgent 数据根目录**: {settings.seeagent_home}
...
def _detect_deploy_mode() -> str:
    ...
    try:
        dist = importlib.metadata.distribution("seeagent")
    ...
    # 检查是否通过 pip 安装
    try:
        importlib.metadata.version("seeagent")
        return "pip install"
    except Exception:
        pass
...
### 提问内容质量要求（硬性规则）
调用 `ask_user` 时，`question` 文本（或 `questions` 中每个问题的 `prompt`）**必须**同时包含以下 4 个要素——只问一句"需要什么时间？"是不合格的：
1. **需要提供的内容**：明确列出需要用户回复的信息点。如有多项，**必须**使用 Markdown 无序列表（`- **字段名**：...`），每项**单独一行**，即 `question` 字符串里每个列表项之间有真实换行符（`\n`）——**不要**把多个列表项写在同一行。
...
```
注意：上面正例里每个 `-` 列表项之间是真实换行。在 JSON 调用 `ask_user` 时，`question` 字段字符串中对应位置也必须是真实换行字符（`\n`），而不是空格。
...
def _build_bp_section(session_id: str = "") -> str:
    """构建 Best Practice 系统提示词段（静态能力描述 + 动态状态）。"""
    try:
        from ..bestpractice.facade import get_dynamic_prompt_section, get_static_prompt_section
        static = get_static_prompt_section()
        if not static:
            return ""
        if session_id:
            dynamic = get_dynamic_prompt_section(session_id)
            if dynamic:
                return f"{static}\n\n{dynamic}"
        return static
    except Exception:
...
def _build_memory_section(
    ...
    from seeagent.memory.types import MEMORY_MD_MAX_CHARS as _MD_MAX
...
def _get_core_memory(memory_manager: Optional["MemoryManager"], max_chars: int = 600) -> str:
    ...
    """
    from seeagent.memory.types import truncate_memory_md
