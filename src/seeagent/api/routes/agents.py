...
async def _hot_register_bot(request: Request, bot: dict) -> None:
    ...
    if gateway is None:
        ...
        return
    try:
        from seeagent.main import _create_bot_adapter
...
async def create_bot(body: BotCreateRequest, request: Request):
    ...
    if bot.get("enabled", True):
        from seeagent.main import apply_im_bot
...
async def update_bot(bot_id: str, body: BotUpdateRequest, request: Request):
    ...
    from seeagent.main import apply_im_bot, remove_im_bot
    if bot.get("enabled", True):
        await apply_im_bot(bot)
    else:
...
async def delete_bot(bot_id: str, request: Request):
    ...
    if deleted:
        from seeagent.main import remove_im_bot
...
async def create_agent_profile(body: ProfileCreateRequest):
    ...
    from seeagent.agents.profile import AgentProfile, AgentType, ProfileStore, SkillsMode
    from seeagent.config import settings
...
async def reset_agent_profile(profile_id: str):
    ...
    from seeagent.agents.presets import get_preset_by_id
    from seeagent.agents.profile import ProfileStore
    from seeagent.config import settings
