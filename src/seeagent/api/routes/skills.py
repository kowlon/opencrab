...
def _notify_skills_changed(action: str = "reload") -> None:
    ...
    try:
        from seeagent.api.routes.websocket import broadcast_event
...
def _apply_allowlist_and_rebuild_catalog(request: Request) -> int:
    ...
    removed = 0
    if loader:
        from seeagent.core.agent import _collect_preset_referenced_skills
...
async def _auto_translate_new_skills(request: Request, install_url: str) -> None:
    ...
    try:
        ...
        from seeagent.skills.i18n import auto_translate_skill
...
async def install_skill(request: Request):
    """安装技能（远程模式替代 Tauri seeagent_install_skill 命令）。
    ...
    try:
        from seeagent.setup_center.bridge import install_skill as _install_skill
    ...
    try:
        from seeagent.setup_center.bridge import _resolve_skills_dir
...
async def uninstall_skill(request: Request):
    ...
    try:
        from seeagent.setup_center.bridge import uninstall_skill as _uninstall_skill
...
async def update_skill_content(skill_name: str, request: Request):
    ...
    """
    from seeagent.core.agent import Agent
    from seeagent.skills.parser import skill_parser
...
async def search_marketplace(q: str = "agent"):
    ...
    from seeagent.llm.providers.proxy_utils import (
        get_httpx_transport,
        get_proxy_config,
    )
