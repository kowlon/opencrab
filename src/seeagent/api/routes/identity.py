...
from seeagent.prompt.budget import estimate_tokens
...
def validate_identity_file(name: str, content: str) -> dict[str, list[str]]:
    ...
    if name == "POLICIES.yaml":
        ...
        from seeagent.memory.types import MEMORY_MD_MAX_CHARS
...
async def compile_identity(request: Request, mode: str = "rules"):
    ...
    if mode == "llm":
        ...
        if brain:
            from seeagent.prompt.compiler import PromptCompiler
    ...
    from seeagent.prompt.compiler import get_compiled_content
...
async def compile_status():
    ...
    from seeagent.prompt.compiler import check_compiled_outdated, get_compiled_content
    compiled = get_compiled_content(identity_dir)
