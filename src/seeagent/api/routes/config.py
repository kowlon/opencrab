...
def _hot_patch_agent_tools(request: Request, *, enable: bool) -> None:
    ...
    if agent is None:
        return
    try:
        from seeagent.tools.definitions.agent import AGENT_TOOLS
        from seeagent.tools.handlers.agent import create_handler as create_agent_handler
...
async def write_agent_mode(body: AgentModeRequest, request: Request):
    ...
    from seeagent.config import runtime_state, settings
    ...
    if body.enabled and not old:
        try:
            from seeagent.main import _init_orchestrator
            await _init_orchestrator()
            from seeagent.main import _orchestrator
...
async def list_providers_api():
    ...
    """
    try:
        from seeagent.llm.registries import list_providers
...
async def list_models_api(body: ListModelsRequest):
    """拉取 LLM 端点的模型列表（远程模式替代 Tauri seeagent_list_models 命令）。
