...
**信息检索类计划优化（必须遵守）**：
- 同一主题不要拆成多个高度相似的 `web_search` 步骤（例如“先搜门票、再搜门票时间、再搜门票政策”这种重复检索）。
- 优先用“1 次综合检索 + 1 次补充验证 + 1 次整理输出”的结构，避免连续重复搜索导致空转。
- 每次 `web_search` 后先判断结果是否已覆盖目标维度；覆盖后直接进入整理，不要机械继续搜。
...
- 如果任务是“攻略/方案/报告/分析”这类信息输出，最终回复必须包含实质内容摘要（关键结论/要点），不能只说“任务已完成”。
