"""
SkillStoreClient — 与 SeeAgent Platform Skill Store 交互的客户端
...
class SkillStoreClient:
    ...
    def _auth_headers(self) -> dict[str, str]:
        headers: dict[str, str] = {"User-Agent": f"SeeAgent/{self._get_version()}"}
    ...
    def _write_origin(skill_dir: Path, install_url: str) -> None:
        ...
        try:
            ...
            (skill_dir / ".seeagent-origin.json").write_text(
                json.dumps(origin, ensure_ascii=False, indent=2), encoding="utf-8"
            )
            # Also write .seeagent-source for compatibility with bridge/frontend matching
            (skill_dir / ".seeagent-source").write_text(install_url, encoding="utf-8")
