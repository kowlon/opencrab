...
提供 Chrome 安装检测、SeeAgent 专用 profile 管理、Cookie 同步等工具函数。
...
def get_seeagent_chrome_profile() -> str:
    """
    获取 SeeAgent 专用的 Chrome profile 目录。
    ...
    profile_dir = base_dir / "SeeAgent" / "ChromeProfile"
...
def sync_chrome_cookies(src_user_data: str, dst_profile: str) -> bool:
    """
    同步用户 Chrome 的 cookies 到 SeeAgent profile。
    只复制关键文件以保持登录状态。
    """
