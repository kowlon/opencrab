...
def _find_bundled_browser_executable() -> str | None:
    ...
    from seeagent.runtime_env import IS_FROZEN
    if not IS_FROZEN:
        return None
...
class BrowserManager:
    ...
    def _setup_browsers_path(self) -> None:
        ...
        _root = os.environ.get("SEEAGENT_ROOT", "").strip()
        _base = Path(_root) if _root else Path.home() / ".seeagent"
    ...
    async def _try_user_chrome(self, headless: bool, *, use_oa_profile: bool) -> bool:
        ...
        if use_oa_profile:
            from .chrome_finder import get_seeagent_chrome_profile, sync_chrome_cookies
            user_data = get_seeagent_chrome_profile()
            if self._chrome_user_data:
                sync_chrome_cookies(self._chrome_user_data, user_data)
            label = "SeeAgent profile"
        else:
