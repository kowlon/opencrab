...
PyInstaller 打包后 sys.executable 指向 seeagent-server.exe 而非 Python 解释器，
...
def _get_seeagent_root() -> Path:
    """获取 SeeAgent 根目录路径 (避免循环导入 config)。

    优先使用 SEEAGENT_ROOT 环境变量，默认 ~/.seeagent。
    """
    import os
    env_root = os.environ.get("SEEAGENT_ROOT", "").strip()
    if env_root:
        return Path(env_root)
    return Path.home() / ".seeagent"


def _get_bundled_internal_python() -> str | None:
    """查找 PyInstaller 打包时捆绑在 _internal/ 目录中的 Python 解释器。

    构建时 seeagent.spec 会将 sys.executable 和 pip 一起复制到 _internal/，
...
def get_python_executable() -> str | None:
    ...
    **只使用项目自带或项目自行安装的 Python，不使用用户系统 Python。**
    ...
    logger.warning(
        "未找到项目自带的 Python 解释器。"
        "已搜索: 工作区 venv → ~/.seeagent/venv → "
        "打包内置 Python。"
        "请重新安装 SeeAgent，确保安装包资源完整。"
    )
    return None
...
def get_channel_deps_dir() -> Path:
    ...
    路径: ~/.seeagent/modules/channel-deps/site-packages
    该目录会被 inject_module_paths() 自动扫描并注入到 sys.path。
    """
    return _get_seeagent_root() / "modules" / "channel-deps" / "site-packages"
...
def _sanitize_sys_path() -> None:
    ...
    seeagent_root = str(_get_seeagent_root())
...
def inject_module_paths() -> None:
    ...
    路径来源（按优先级）：
    1. SEEAGENT_MODULE_PATHS 环境变量 — Tauri 端通过此变量传递已安装模块路径
    2. 扫描 ~/.seeagent/modules/*/site-packages — 兜底机制
    ...
    # 来源 1：从 SEEAGENT_MODULE_PATHS 环境变量读取（Tauri 端设置）
    env_paths = os.environ.get("SEEAGENT_MODULE_PATHS", "")
    if env_paths:
    ...
    # 来源 2：扫描 ~/.seeagent/modules/*/site-packages（兜底）
    # 跳过已内置到 core 包的模块，避免外部旧版本与内置版本冲突
    _BUILTIN_MODULE_IDS = {"browser"}
    modules_base = _get_seeagent_root() / "modules"
...
def inject_module_paths_runtime() -> int:
    ...
    # 扫描 ~/.seeagent/modules/*/site-packages
    modules_base = _get_seeagent_root() / "modules"
    if modules_base.exists():
        for module_dir in modules_base.iterdir():
            if not module_dir.is_dir():
