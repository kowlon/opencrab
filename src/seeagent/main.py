...
async def _init_orchestrator():
    ...
    try:
        from seeagent.agents.presets import ensure_presets_on_mode_enable
...
def _ensure_channel_deps() -> None:
    ...
    ``~/.seeagent/modules/channel-deps/site-packages``，与外部 Python
    ...
    from seeagent.runtime_env import get_channel_deps_dir, get_python_executable, IS_FROZEN
...
def main(
    ...
    if version:
        ...
        console.print(f"SeeAgent v{__version__}")
        raise typer.Exit(0)
    ...
    if ctx.invoked_subcommand is None:
        ...
        if not has_endpoint:
            ...
            console.print(
                "请设置 ANTHROPIC_API_KEY，或运行 'seeagent init' 配置 data/llm_endpoints.json"
            )
            raise typer.Exit(1)
...
def serve(
    ...
    def _write_heartbeat():
        ...
        try:
            ...
            from seeagent import __git_hash__, __version__
            data = {
                "pid": os.getpid(),
                "timestamp": time.time(),
    ...
    async def _serve():
        ...
        logger.info(f"SeeAgent {_version_str} starting...")

        console.print(
            Panel(
                f"[bold]SeeAgent 服务模式[/bold]\n\n"
                f"版本: {_version_str}\n"
        ...
        _api_fatal = False
        try:
            from seeagent.api.server import start_api_server
        ...
        if dev:
            async def _file_watcher():
                try:
                    ...
                    src_dir = Path(__file__).resolve().parent  # src/seeagent/
    ...
    while first_run or cfg._restart_requested:
        ...
        if cfg._restart_requested:
            ...
            try:
                from seeagent.api.server import API_HOST, API_PORT, wait_for_port_free
