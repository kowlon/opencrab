...
| 产品术语 | 英文 | 已有 / 新增 | 代码实体 | 说明 |
|---------|------|-----------|---------|------|
| 触发类型 | Trigger Type | **扩展已有** | `TriggerType`（`scheduler/task.py`） | 已有 `ONCE` / `INTERVAL` / `CRON`，需新增 `COMMAND` / `EVENT` / `CONTEXT` / `UI_CLICK` |
...
| 子任务进度 | SubTask Progress | **新增** | `subtaskStatus: 'pending' \| 'current' \| 'done'` | 总任务卡片中每个子任务的进度标记 |
...
最佳实践支持 **5 种触发方式**，其中前 3 种需**扩展**现有 `TriggerType` 枚举：

| # | 触发方式 | 映射到 `TriggerType` | 示例 | 说明 |
|---|---------|---------------------|------|------|
| 1 | **系统指令** | `COMMAND`（新增） | "执行市场调研" | 用户明确指令触发 |
| 2 | **事件驱动** | `EVENT`（新增） | "当新数据到达时执行" | 外部事件回调触发 |
| 3 | **上下文推断** | `CONTEXT`（新增） | 聊天中自动识别触发条件 | 根据对话上下文智能匹配 |
| 4 | **定时调度** | `CRON`（已有） | "每周一凌晨1点" | 复用 `ScheduledTask` |
| 5 | **页面点击** | `UI_CLICK`（新增） | 用户在 UI 上选择 | 手动选取触发 |
...
| **触发方式** | 扩展 `TriggerType`：`COMMAND` / `EVENT` / `CONTEXT` + 已有 `CRON` + `UI_CLICK` |
