...
class TestBusyLock:
    async def test_mark_and_clear(self):
        ...
        assert await _bp_mark_busy("sess-1", "bp_start")
        assert not await _bp_mark_busy("sess-1", "bp_next")  # Already locked
        _bp_clear_busy("sess-1")
        assert await _bp_mark_busy("sess-1", "bp_next")  # Free now
        _bp_busy_locks.clear()
    ...
    async def test_renew_updates_timestamp(self):
        ...
        _bp_busy_locks.clear()
        await _bp_mark_busy("sess-1", "bp_start")
        _, ts1 = _bp_busy_locks["sess-1"]
        await asyncio.sleep(0.01)
        _bp_renew_busy("sess-1")
        _, ts2 = _bp_busy_locks["sess-1"]
        assert ts2 > ts1
        _bp_busy_locks.clear()
