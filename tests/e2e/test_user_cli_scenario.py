...
Simulates what happens when a user runs SeeAgent from the command line:
- `seeagent` starts interactive mode
...
class TestCLIStartup:
    """User runs `seeagent` or `python -m seeagent` for the first time."""

    def test_module_importable(self):
        """The package can be imported without errors."""
        result = subprocess.run(
            [sys.executable, "-c", "import seeagent; print('OK')"],
    ...
    def test_help_flag(self):
        ...
        combined = result.stdout + result.stderr
        assert "usage" in combined.lower() or "help" in combined.lower() or "seeagent" in combined.lower()
...
class TestCLISubcommands:
    ...
    def test_status_command_exists(self):
        """'seeagent status' should be a recognized command."""
        result = subprocess.run(
    ...
    def test_init_command_exists(self):
        """'seeagent init' should be a recognized command."""
        result = subprocess.run(
    ...
    def test_selfcheck_command_exists(self):
        """'seeagent selfcheck' should be a recognized command."""
        result = subprocess.run(
    ...
    def test_serve_command_exists(self):
        """'seeagent serve --help' should show API server options."""
        result = subprocess.run(
            [sys.executable, "-m", "seeagent", "serve", "--help"],
            capture_output=True, text=True, timeout=15,
        )
        combined = result.stdout + result.stderr
