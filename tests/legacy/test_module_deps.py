...
class TestInjectModulePaths:
    ...
    def test_inject_adds_existing_site_packages(self, tmp_path):
        ...
        with patch("seeagent.runtime_env._get_seeagent_root", return_value=tmp_path):
            original_path_len = len(sys.path)
            from seeagent.runtime_env import inject_module_paths_runtime
...
class TestRegisterDllDirectories:
    ...
    def test_registers_torch_lib(self, tmp_path):
        ...
        try:
            ...
            from seeagent.runtime_env import _register_dll_directories
...
class TestImportHelper:
    ...
    def test_all_module_packages_mapped(self):
        ...
        from seeagent.tools._import_helper import _PACKAGE_MODULE_MAP
    ...
    def test_frozen_hint_mentions_setup_center(self):
        ...
        from seeagent.tools._import_helper import _build_hint

        with patch("seeagent.tools._import_helper.IS_FROZEN", True):
    ...
    def test_dev_hint_mentions_pip(self):
        ...
        with patch("seeagent.tools._import_helper.IS_FROZEN", False):
            hint = _build_hint("sentence_transformers")
...
class TestPlaywrightBrowsersPath:
    ...
    def test_sets_browsers_path_when_dir_exists(self, tmp_path):
        ...
        browsers_dir = tmp_path / ".seeagent" / "modules" / "browser" / "browsers"
        ...
        try:
            with patch("pathlib.Path.home", return_value=tmp_path):
                ...
                if "PLAYWRIGHT_BROWSERS_PATH" not in os.environ:
                    check_dir = Path.home() / ".seeagent" / "modules" / "browser" / "browsers"
                    if check_dir.is_dir():
...
class TestPostInstallHooks:
    ...
    def test_playwright_browsers_path_set_at_launch(self):
        ...
        # 确认在后端启动函数中（cmd.env SEEAGENT_MODULE_PATHS 附近）有设置
        lines = content.split("\n")
        found_launch_pw_path = False
        in_launch_region = False
        for line in lines:
            if "SEEAGENT_MODULE_PATHS" in line and "cmd.env" in line:
        ...
        assert found_launch_pw_path, (
            "启动后端进程时（cmd.env SEEAGENT_MODULE_PATHS 之后）应设置 PLAYWRIGHT_BROWSERS_PATH"
        )
...
class TestMemoryRetrievalFallback:
    ...
    def test_retriever_falls_back_to_keyword(self):
        ...
        from seeagent.prompt.retriever import _search_related_memories
    ...
    def test_retriever_async_falls_back(self):
        ...
        from seeagent.prompt.retriever import async_search_related_memories
...
class TestVectorStoreRetryReset:
    ...
    def test_lazy_import_globals_reset_on_retry(self):
        ...
        import seeagent.memory.vector_store as vs_module
    ...
    def test_lazy_import_retries_when_reset_to_none(self):
        ...
        try:
            ...
            with patch("seeagent.memory.vector_store.inject_module_paths_runtime",
